...
class Agent:
    ...
    def collide(self, other):
        ...
        if d < self.radius + other.radius:
            ...
            # d = (self.radius - ovlp  + other.radius - ovlp ) + ovlp
            # ovlp = (self.radius + other.radius - d)
            # ovlp/2 = (self.radius + other.radius - d)/2
    ...
    def cluster(self, other):
        ...
        if d > self.radius + other.radius:
            ...
            # change vector magnitude to 1
            v.Unitize()
            # set magnitude to half the extend distance
            # d = self.radius + other.radius + extd
            # extd = d - self.radius -  other.radius
            # extd/2 = (d - self.radius - other.radius)/2
...
for i in range(max_iters):
    ...
    for j,agent_1 in enumerate(agents):
        ...
        for agent_2 in agent_1.neighbors:
            total_count = total_count + agent_1.cluster(agent_2)
        
        # collide with all agents after agent in list
        for agent_2 in agents[j+1:]:
            total_count = total_count + agent_1.collide(agent_2)
