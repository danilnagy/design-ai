...
for i, row in enumerate(pts[:-1]): ## it returns all elements [:] except the last one -1
    for j, pt_1 in enumerate(row[:-1]): ## enumerate gives us access to two variables for each loop, one that represents the index of an item in a list, and one that represents the item itself
        ...
        getrc, u, v = srf.ClosestPoint(pt_1)
        getint, pt_int = rh.Intersect.Intersection.LinePlane(rh.Line(pt_1, srf.NormalAt(u, v), 100), pl_1)
        pt_1a = rh.Line(pt_1, srf.NormalAt(u, v), 100).PointAt(pt_int)   
        
        pl_2 = rh.Plane(pt_1, pt_3, pt_4)
        t_2 = rh.Transform.PlanarProjection(pl_2) ## making sure we get flat panels
        getrc, u, v = srf.ClosestPoint(pt_2)
        getint, pt_int = rh.Intersect.Intersection.LinePlane(rh.Line(pt_2, srf.NormalAt(u, v), 100), pl_2)
        pt_2a = rh.Line(pt_2, srf.NormalAt(u, v), 100).PointAt(pt_int)
        
        pl_3 = rh.Plane(pt_1, pt_2, pt_4)
        t_3 = rh.Transform.PlanarProjection(pl_3) ## making sure we get flat panels
        getrc, u, v = srf.ClosestPoint(pt_3)
        getint, pt_int = rh.Intersect.Intersection.LinePlane(rh.Line(pt_3, srf.NormalAt(u, v), 100), pl_3)
        pt_3a = rh.Line(pt_3, srf.NormalAt(u, v), 100).PointAt(pt_int)
        
        pl_4 = rh.Plane(pt_1, pt_2, pt_3)
        t_4 = rh.Transform.PlanarProjection(pl_4) ## making sure we get flat panels
        getrc, u, v = srf.ClosestPoint(pt_4)
        getint, pt_int = rh.Intersect.Intersection.LinePlane(rh.Line(pt_4, srf.NormalAt(u, v), 100), pl_4)
        pt_4a = rh.Line(pt_4, srf.NormalAt(u, v), 100).PointAt(pt_int)
