...
for i, row in enumerate(pts[:-1]):
    for j, pt_1 in enumerate(row[:-1]):
        ...
        ## I am not sure of how this was done and used the help of a classmate.
        ## I am perhaps little unclear on how we iterate over the point dictionary
        ...
        and (pt_1.DistanceTo(pt_lt) < pt_4.DistanceTo(pt_4))): # 1 smallest
        ...
        if pt_1.DistanceTo(pt_lt) > 0.01:
            polys[-1]["edge"].append(rh.PolylineCurve([pt_2, pt_lt, pt_1, pt_2]))
            polys[-1]["edge"].append(rh.PolylineCurve([pt_4, pt_1, pt_lt, pt_4]))
        elif ((pt_2.DistanceTo(pt_2t) < pt_1.DistanceTo(pt_lt)) and (pt_2.DistanceTo(pt_2t) < pt_3.DistanceTo(pt_3t))
        and (pt_2.DistanceTo(pt_2t) < pt_4.DistanceTo(pt_4t))): # 2 smallest
            ...
            planar_poly = rh.PolylineCurve( [pt_1, point, pt_3, pt_4, pt_l])
        if pt_2.DistanceTo(pt_2t) > 0.01:
            polys [-1] ["edge"].append(rh.PolylineCurve([pt_l, pt_2t, pt_2, pt_l]))
        ...
        if pt_4.DistanceTo(pt_4t) > 0.01:
            polys[-1]["edge"].append(rh.PolylineCurve([pt_3, pt_4, pt_4, pt_3]))
            polys[-1]["edge"].append(rh.PolylineCurve([pt_1, pt_4, pt_4, pt_1]))
            polys[-1]["planar"] = planar_poly ## store planar poly with a different key
        original = [poly["original"] for poly in polys]
        planar= [poly["planar"] for poly in polys]
        edge= [poly["edge"] for poly in polys]
