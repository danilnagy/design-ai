...
class Agent:
    ...
    #add room and adjacency to def
    def __init__(self, pt, r, room, adj):
...
#add rooms and adjacencies to function
def run(pts, radii, max_iters, alpha, adjacencies):

    rooms = list(adjacencies.keys())
    agents = []

    for i, pt in enumerate(pts):
        my_agent = Agent(pt, radii[i], rooms[i], adjacencies[rooms[i]])
        agents.append(my_agent)

    # for each agent in the list, check adjs, then add neighbor if match
    for i in range(len(agents)):
        # agents[i].add_neighbor(agents[i-1])
        for j in range(len(agents)):
            if agents[j-1].room in agents[i].adj:
    ...
    for agent in agents:
        ...
        names.append(agent.room)
        for i in range(len(agent.neighbors)):
            print(agent.room + " is by " + agent.neighbors[i].room)
