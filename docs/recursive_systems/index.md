...
def grow(pts, params): ## input to the grow() function is now a list of points
    ...
    if param == 1:
        ...
        new_pt_1 = rh.Point3d(start_pt)
        new_pt_1.Transform(rh.Transform.Translation(1,1,1))
        lines.append(rh.Line(start_pt, new_pt_1))
        pts.append(new_pt_1) ## the new point is added to the list of points

        new_pt_2 = rh.Point3d(start_pt)
        new_pt_2.Transform(rh.Transform.Translation(-1,-1,1))
        lines.append(rh.Line(start_pt, new_pt_2))
        pts.append(new_pt_2) ## the new point is added to the list of points
...
branches = grow([rh.Point3d(0,0,0)], params) ## passing the starting point as the single item
```
...
def split_space(curve, dir, param):
    ...
    #PSEUDOCODE : use Vector3d.Rotate on vecs to transform the two vectors by an ANGLE aka [dir]*(2*math.pi) along their respective axes
    # something like ....
    vecsRt = [rh.Vector3d.Rotate([dir]*(2*math.pi),(1,0,0)), rh.Vector3d.Rotate([dir]*(2*math.pi),(0,1,0))]
    
    #print(vecs)
    #print('vecs')
    
    # create a vector to position the split line based on the two parameters, substituting the rotated vector as first input
    vec_1 = vecsRt[dir] * dims[dir] * param
    ...
    # create a vector in the other direction the full extent of the bounding box, substituting the second rotated vector
    vec_2 = vecsRt[other_dir] * dims[other_dir]
