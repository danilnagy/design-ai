...
def split_space(curve, dir, param):
    ...
    #Turns dirs into index 
    ## NOTE: Needs to make sure dirs is not an int but float in the rhino file
    ...
    other_dir = abs(index-1)
    #gets how far the points need to move 
    angel = [math.tan(dir * math.pi)* dims[other_dir], math.tan(dir * math.pi) * dims[other_dir]]

    
    # create a vector to position the split line based on the two parameters
    vec_1 = vecs[index] * dims[index] * param
    ...
    #rotate by moving the two points in opposite directions
    
    vec_3 = vecs[index] * angel[index]
    new_pt_2.Transform(rh.Transform.Translation(vec_3/2))
    new_pt_1.Transform(rh.Transform.Translation(vec_3/2*-1))
