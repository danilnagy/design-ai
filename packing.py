...
class Agent:
    ...
    def __init__(self, pt, r,nam,ajc):

        self.cp = pt
        self.radius = r
        self.neighbors = []
        self.names=nam
        self.adjacencies =ajc
...
def run(pts, radii,names, max_iters, alpha, ajc):
    ...
    for i, pt in enumerate(pts):
        my_agent = Agent(pt, radii[i],names[i],ajc[i])
        agents.append(my_agent)


    # for each agent in the list, add adjancies as its neighbor
    ...
    for i in range(max_iters):
        ...
        for j, agent_1 in enumerate(agents):
            ...
            for agent_2 in agents[j+1:]:
                ...
                total_amount += agent_1.collide(agent_2, alpha/3)
